...
function documentUser()
{
    $.getJSON("/document_user/" + String(current_user_id), function(data)
    {
        ...
        $.each(data, function()
        {
            ...
            documentContainer.innerHTML += documentTemplate.replace("Text.", "Name: " + data[i].name).replace("ID.",  "ID: " + data[i].id).replace("Date.", data[i].date).replace("Id.", data[i].id).replace("Id.", data[i].id);
            i++;
        });
    });
